...
RULES & CONSTRAINTS:
1. Event Detection: Determine whether the poster announces an upcoming event (isEvent: true) or is an announcement, recruitment, sponsor promotion, competition result, or recap (isEvent: false).
2. Post Kind: Classify postKind into strictly one of: "event", "announcement", "recruitment", "sponsor", "result", "other".
3. Normalize Venues: Map common acronyms to their full names (e.g., "OAT" -> "Open Air Theater", "LHC" -> "Lecture Hall Complex", "Dogra" -> "Dogra Hall", "SAC" -> "Student Activity Centre").
4. Time Formatting: Convert all times to 24-hour HH:MM format. If missing, output null (do NOT fabricate times).
5. Date Formatting: Convert all dates to YYYY-MM-DD. Assume the current year is 2026 unless specified otherwise. If missing, output null.
6. Categorization: You MUST categorize the event into strictly ONE of these exact tags: ${JSON.stringify(ALLOWED_CATEGORIES)}.
7. Missing Data: If a field genuinely cannot be deduced from the image or caption, output null (do not guess blindly).
8. Host Identification: If you can identify the organizing club or board, use their Instagram handle format (e.g., @pac_iitd, @brcaiitd). If unsure, use the full name.

OUTPUT SCHEMA (STRICT JSON ONLY):
{"isEvent": true, "postKind": "event", "title": "String", "host": "String", "date": "YYYY-MM-DD", "startTime": "HH:MM", "endTime": "HH:MM", "venue": "String", "category": "String", "summary": "String", "confidenceScore": 0.0}`;
