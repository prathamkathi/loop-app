...
export default function EventCard({ event, saved, onToggleSave, onPress, isPast, featured = false }: Props) {
  ...
  const poster = <View style={[styles.poster, { backgroundColor: colors.highlight }, horizontal ? styles.posterHorizontal : { aspectRatio: featured ? 1.45 : 1.3 }]}>
    {event.image && failedImage !== event.image
      ? <Image source={{ uri: getOptimizedImageUrl(event.image) }} style={StyleSheet.absoluteFill} contentFit="contain" cachePolicy="memory-disk"
          accessibilityLabel={event.title + ' event poster'} onError={() => setFailedImage(event.image)} recyclingKey={event.image} />
      : <View style={styles.posterFallback}><Icon size={54} color={colors.primary} weight="duotone" /><Text style={[typography.labelMd, { color: colors.primary }]}>{category.label}</Text></View>}
  </View>;
  return <>
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <Pressable onPress={onPress} accessibilityRole="button" accessibilityLabel={'View details for ' + event.title}
        ...
        <View style={[styles.body, horizontal && styles.featureBody]}>
          <View style={styles.categoryRow}><Icon size={16} color={colors.primary} weight="duotone" />
      ...
      <View style={[styles.actions, { borderTopColor: colors.border }]}>
        <View style={styles.actionLinks}>
          {event.image ? <Pressable onPress={() => setShowLightbox(true)} accessibilityRole="button" accessibilityLabel={'View full poster for ' + event.title} style={styles.action}>
          ...
          {event.actionUrl ? <Pressable onPress={() => openExternalLink(event.actionUrl!)} accessibilityRole="link" accessibilityLabel={'Open official link for ' + event.title} style={styles.action}>
            <ArrowUpRight size={18} color={colors.primary} /><Text style={[typography.labelSm, { color: colors.primary }]}>Official link</Text>
          </Pressable> : contact ? <Pressable onPress={() => openWhatsApp(contact.phone, contact.name, event.title)} accessibilityRole="link" accessibilityLabel={'Contact ' + (contact.name || 'organizer') + ' on WhatsApp'} style={styles.action}>
...
const styles = StyleSheet.create({
  ...
  posterFallback: { flex: 1, justifyContent: 'center', alignItems: 'center', gap: 14, padding: 24 },
